...
from client_mp import DecentralizedClient, constr_prob_matrix
from distributed_mp import run_decentralized_fl
...
def main(cfg: DictConfig):
    ...
    num_parallel = cfg['federation'].get('num_parallel', None)
    
    # If num_parallel not specified, default to available CPU cores
    if num_parallel is None:
        if device.type == 'cuda':
            # Set to 2 for GPU to avoid memory issues
            num_parallel = 2
        else:
            # Use number of CPU cores
            num_parallel = max(1, os.cpu_count() - 1)
    
    # Run decentralized federated learning with multiprocessing
    print("Starting decentralized federated learning with multiprocessing...")
    try:
        results = run_decentralized_fl(
            clients=clients,
            rounds=cfg['federation']['rounds'],
            rounds_patience=cfg['federation']['rounds_patience'],
