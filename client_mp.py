...
class DecentralizedClient:
    ...
    def __init__(
        ...
        if len(self.neighbors_sim) > 0:
            self.neighbors_proba = np.exp(np.array(self.neighbors_sim) / self.tau) 
            sum_proba = np.sum(self.neighbors_proba)
            if sum_proba > 0:
                self.neighbors_proba = self.neighbors_proba / sum_proba
            else:
                # If all probabilities are zero, use uniform distribution
                self.neighbors_proba = np.ones_like(self.neighbors_proba) / len(self.neighbors_proba)
        else:
            self.neighbors_proba = np.array([])
        
        # Initialize storage for received models
    ...
    def get_parameters(self):
        ...
        return {name: param.clone().detach().cpu() for name, param in self.model.named_parameters()}
    ...
    def set_parameters(self, params):
        ...
        with torch.no_grad():
            ...
            # Re-initialize optimizer to reflect new parameters
            self.optimizer = getattr(optim, self.optimizer_name)(self.model.parameters(), lr=self.lr)
    
    def select_neighbors(self):
        """Select subset of neighbors for communication"""
        if not self.neighbors or len(self.neighbors) == 0:
            return []
        
        # Based on the similarity with your neighbors do the selection
        num_to_select = max(1, ceil(len(self.neighbors) * self.selection_ratio))
        num_to_select = min(num_to_select, len(self.neighbors))
        
        return np.random.choice(self.neighbors, num_to_select, 
...
def constr_prob_matrix(clients):
    ...
    # Check that rows sum to 1, allowing for small floating point errors
    row_sums = P.sum(axis=1)
    valid_rows = np.isclose(row_sums, 1.0) | (row_sums == 0)
    if not np.all(valid_rows):
        print("Warning: Some rows of probability matrix do not sum to 1")
    return P